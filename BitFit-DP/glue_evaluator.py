...
class CustomBaseModelWrapper(nn.Module):
    def __init__(self, model_name, return_dict=False):
        super(CustomBaseModelWrapper, self).__init__()
        self.model = AutoModelForSequenceClassification.from_pretrained(model_name, return_dict=return_dict)
...
class GLUEvaluator:
    ...
    def __init__(self, task_name, model_name, device):
        ...
        self.device = torch.device("mps") if device is not None else None
    ...
    def training_preparation(self, learning_rate, optimizer, encoder_trainable, trainable_components=None,
        ...
        self.model = CustomModelWrapper(self.model_name, config=config)
    ...
    def train_and_evaluate(self, num_epochs, output_path=None, evaluation_frequency=1, noise_multiplier=1.1, max_grad_norm=1.5):
        ...
        for epoch in range(num_epochs):
            ...
            self._train(self.data_loaders['train'], epoch, noise_multiplier, max_grad_norm)
    ...
    def export_model_test_set_predictions(self, output_path):
        ...
        for prediction_file_name, dataloader in test_data_loaders.items():
            ...
            for batch in dataloader:
                ...
                if self.device is not None:
                    batch = tuple(obj.to(self.device) for obj in batch)
    ...
    def plot_terms_changes(self, output_path=None):
        ...
        if 'distilbert' in self.model_name:
            base_model = CustomBaseModelWrapper(self.model_name, return_dict=True).model.distilbert
            # print("base_model:\n")
            # print(base_model)
            fine_tuned_model = self.model.cpu()
        else:
            base_model = CustomBaseModelWrapper(self.model_name, return_dict=True).bert
            fine_tuned_model = self.model.cpu().bert

        # print("self.model:\n")
        # print(self.model.model.distilbert.)
        num_layers = self.def_config.num_hidden_layers
        ...
        if values_map.size > 0:  # Check if the array is non-empty
            for change in changes:
            ...
            print("Warning: Empty data array. Cannot plot heatmap.")
    ...
    def _train(self, train_dataloader, epoch, noise_multiplier,max_grad_norm):
        ...
        if privacy_engine.is_compatible(module=self.model, optimizer=self.optimizer, data_loader=train_dataloader):
            print("Privacy Engine is compatible with the given components")
            ...
            print("Privacy Engine is NOT compatible with the given components")
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            print(f'EPOCH: {epoch}   TRAIN: {trained_samples}/{n}   LOSS: {round(loss_sum / (step + 1), 3)}   GradNorm: {round(grad_norm, 3)}\r', end='')
        print('')
