...
def _parse_args():
    ...
    parser.add_argument('--model-name', '-m', type=str, default='bert-base-cased', help='model-name to evaluate with.',
                        choices={'bert-base-cased', 'distilbert-base-cased', 'bert-large-cased', 'roberta-base'})
    ...
    parser.add_argument('--noise-multiplier', type=float, default=1.1,
                        help='The ratio of the standard deviation of the Gaussian noise to the L2-sensitivity of the function to which the noise is added ')
    parser.add_argument('--max-grad-norm', type=float, default=1.5,
                        help='Flag to indicate if the input tensor to the corresponding module has the first dimension representing the batch.')
...
def main():
    ...
    # train and evaluate
    evaluator.train_and_evaluate(args.epochs, args.output_path, noise_multiplier=args.noise_multiplier, max_grad_norm=args.max_grad_norm)
